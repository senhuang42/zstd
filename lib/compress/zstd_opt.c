                    ...
                    U32 const mls /* template */)
{
    ...
    /* HC3 match finder: use a hash table to find matches of length 3 for speed purposes. */
    ...
    if (mnum > 0 && ms->ldmSeqStore && ms->ldmSeqStore->size > 0) {
        ...
        if (ldmIndex >= 0) {
            ...
            if (matchLength >= bestLength) {
                ...
                /* rawSeqStore generated by multithreaded compression may result in match lengths longer than a block.
                 * So there is a possibility that we must split the sequence across block boundaries.
                 * ZSTD_ldm_maybeSplitSequence() mutates the sequence at "pos" and returns a valid sequence.
                 */
                ...
                if (ldmSeqStore->pos != ldmIndex) {
                    ...
                    if (finalSeq.offset == 0) {
                         ...
                         */
                        ldmSeqStore->bytesRead += remainingBytes;
                        ldmSeqStore->seq[ldmSeqStore->pos].litLength = 0;
                    } else {
                        ldmSeqStore->bytesRead += finalSeq.matchLength;
